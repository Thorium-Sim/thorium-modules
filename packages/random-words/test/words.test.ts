import randomWords from '../src/index';
...
describe('random-words', function() {
  ...
  it('returns result of variable length when called with min: 5 and max: 10', function() {
    var lengths: { [key: number]: boolean } = {};
    for (var i = 0; i < 100; i++) {
      var words = randomWords({ min: 5, max: 10 });
  ...
  it('should support a special random int function', function() {
    var words = randomWords({ exactly: 10, randomFloat: () => 0.5 });
    var otherWords = randomWords({ exactly: 10, randomFloat: () => 0.5 });
