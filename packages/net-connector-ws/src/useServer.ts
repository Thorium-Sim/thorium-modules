...
import { makeServer, ServerOptions} from '@thorium-sim/net-connector';
...
export function useServer(
  ...
  ws.on('connection', (socket, request) => {
    ...
    socket.once('close', (code, reason) => {
      ...
      if (!isProd && code === 1002)
        console.warn(
          `WebSocket protocol error occurred. It was most likely caused due to an ` +
            `unsupported sub-protocol "${socket.protocol}" requested by the client. ` +
            `@thorium-sim/net-connector-ws implements exclusively the thorium sim sub-protocol, ` +
